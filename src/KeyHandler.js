...
class KeyHandler {
    ...
    registerKey(eventType, key, callback) {
        ...
        if (this.eventTypes[eventType].length === 0) {
            document.addEventListener(eventType, this.keyRouterBound);
    ...
    unregisterKey(eventType, key, callback = null) {
        if (!this.eventTypes.hasOwnProperty(eventType)) {
            console.warn('Key Handler: Cannot handle event type: ' + eventType);
            return;
        }
        ...
        if (~removee)
            this.eventTypes[eventType].splice(removee, 1);
        if (this.eventTypes[eventType].length === 0) {
            document.removeEventListener(eventType, this.keyRouterBound);
